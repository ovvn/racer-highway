...
// NOTE: _sender._bufferedBytes is a ws v2 internal.
// Re-verify this property exists before upgrading ws.
function getBufferedBytes(client) {
  const senderBytes =
    (client._sender && client._sender._bufferedBytes) || 0;
  const socketBytes =
    (client._socket && client._socket.bufferSize) || client.bufferedAmount || 0;
  return senderBytes + socketBytes;
}
 ...
 */
function createWebSocketStream(client, options) {
  let stream = new ClientStream(client, options);
...
function ClientStream(client, options) {
  this.client = client;
...
ClientStream.prototype._write = function(chunk, encoding, callback) {
  let self = this;
  if (!this.client) {
    return callback(new Error('WebSocket already destroyed'));
  }
  ...
  const MAX_MB = this.options.wsMaxBufferMB || 5;
  const LIMIT = MAX_MB * 1024 * 1024;
  const buffered = getBufferedBytes(this.client);

  if (buffered > LIMIT) {
    const currentMB = (buffered / 1024 / 1024).toFixed(2);
...
ClientStream.prototype._stopClient = function() {
  ...
  if (!this.destroyed) {
    // triggers 'close' reliably, better than push(null) + manual flags
    this.destroy(new Error('client terminated'));
  }
};
