...
Socket.prototype._createWebSocket = function() {
  ...
  this._socket.onerror = this._ws_onerror.bind(this);
...
Socket.prototype._ws_onclose = function(event) {
  ...
  this._syncState();
  console.log('[racer-highway] WebSocket: connection is broken', event);
...
Socket.prototype._ws_onerror = function(error) {
  console.error('[racer-highway] WebSocket error:', error);
  this.reconnect();
};
  ...
  if (this._type === 'websocket') {
    if (this._socket.readyState === WebSocket.OPEN && this._messageQueue.length === 0) {
      try {
        ...
        console.error('[racer-highway] Error sending data:', e);
        this._messageQueue.push(data);
        // Think about force closing a websocket and reconnect
        // this.close();
      }
    } else {
      if (this._messageQueue.length < MAX_QUEUE) {
        this._messageQueue.push(data);
      } else {
        console.warn('[racer-highway] Message queue full, dropping message');
      }
    }
  } else {
    try {
...
Socket.prototype.close = function() {
  try {
    ...
    console.error('[racer-highway] Error while closing WebSocket:', error);
  } finally {
    // Ensure cleanup happens regardless of errors
    if (this._socket.timer) {
      clearInterval(this._socket.timer);
    }
    this._socket = null;
  }
};
...
Socket.prototype.reconnect = function(){
  if (this._type === 'websocket' && this.readyState === this.CLOSED) {
    console.log('[racer-highway] Attempting to reconnect...');
    setTimeout(() => {
      this._createWebSocket();
    }, this._getTimeout());
  }
